...
class RWA_DDCW_Classifier(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    def fit_single_sample(self, X, y, classes=None, sample_weight=None):
        ...
        classes: list
            List of all existing classes. Tento parameter sa teraz berie z self._classes.

        sample_weight: numpy.ndarray of shape (n_samples), optional (default=None)
            Samples weight. If not provided, uniform weights are assumed.

        """
        # Uistíme sa, že _classes je nastavené
        if self._classes is None and classes is not None:
            self._classes = list(classes)
        # Ak classes neprídu, ale už ich máme uložené, použijeme ich
        elif classes is None and self._classes is not None:
            classes = self._classes

        if self.p <= 0:
            for exp in self.experts:
                # Použijeme self._classes pre prvý tréning
        ...
        if self.p <= 0:
            self.X_batch = np.zeros((self.window_size, D), dtype=float) # Zmenené na float pre TF-IDF
        ...
        # Aktualizujeme počet tried, ak sa objaví nová
        known_classes_count = len(self._classes) if self._classes is not None else 0
        self.num_classes = max(known_classes_count, int(np.max(y)) + 1)
        if self._classes is not None and len(self._classes) < self.num_classes:
            # Ak sa objavila nová trieda, rozšírime zoznam
        ...
        for i, exp in enumerate(self.experts):
            y_hat = exp.estimator.predict(X)
            # Ošetrenie prípadu, kedy y_hat môže byť prázdne
            ...
            if len(exp.weight_class) < self.num_classes:
                exp.weight_class = np.pad(exp.weight_class, (0, self.num_classes - len(exp.weight_class)), 'constant', constant_values=(1 / len(self.experts) if len(self.experts) > 0 else 1.0))
        ...
        if self.p >= self.window_size:
            ...
            # --- ZAČIATOK NOVÉHO HYBRIDNÉHO PRAVIDLA ---

            weakest_by_weight_index = None
            weakest_by_weight_score = float('inf')
            
            # Najprv nájdeme najslabšieho experta podľa pôvodnej metriky váh
            # (Tento kód bol pôvodne v inej slučke, teraz ho dáme sem)
            for i, exp in enumerate(self.experts):
                current_score = sum(exp.weight_class)
                if current_score <= weakest_by_weight_score:
                    weakest_by_weight_index = i
                    weakest_by_weight_score = current_score
            
            # Teraz vypočítame RWA pre všetkých expertov
            expert_rwa_scores = []
            for i in range(len(self.experts)):
                expert_predictions = self.y_batch_experts[i, :]
                rwa = calculate_rwa(self.y_batch, expert_predictions, self._classes)
                expert_rwa_scores.append(rwa)

            # Ochranný mechanizmus
            final_weakest_index = weakest_by_weight_index
            
            if len(expert_rwa_scores) > 2 and weakest_by_weight_index is not None:
                # Zistíme indexy TOP 2 expertov podľa RWA
                top_rwa_indices = np.argsort(expert_rwa_scores)[-2:]

                # Ak je náš najslabší expert (podľa váh) zároveň jedným z TOP RWA expertov...
                if weakest_by_weight_index in top_rwa_indices:
                    print(f"DEBUG (vzorka {self.epochs}): OCHRANA experta {weakest_by_weight_index} (RWA: {expert_rwa_scores[weakest_by_weight_index]:.4f}). Hľadám náhradu.")
                    
                    # Hľadáme druhého najslabšieho, ktorý nie je chránený
                    # Vytvoríme si zoznam kandidátov na vyhodenie
                    candidate_indices = [i for i, exp in enumerate(self.experts) if i not in top_rwa_indices]
                    
                    if candidate_indices:
                        # Z kandidátov vyberieme toho s najnižším súčtom váh
                        candidate_scores = [sum(self.experts[i].weight_class) for i in candidate_indices]
                        final_weakest_index = candidate_indices[np.argmin(candidate_scores)]
                        print(f"DEBUG: Namiesto neho odstraňujem experta {final_weakest_index}.")
                    else:
                        # Ak sú všetci experti chránení (divný prípad), nespravíme nič
                        final_weakest_index = None

            # --- KONIEC NOVÉHO HYBRIDNÉHO PRAVIDLA ---

            self._normalize_weights_class(sum_weight_class)

            if np.any(y_hat_final != y):
                # Teraz použijeme `final_weakest_index`
                if len(self.experts) >= self.max_estimators and final_weakest_index is not None:
                    if final_weakest_index < len(self.experts):
                        self.experts.pop(final_weakest_index)
                
                # ... zvyšok logiky pre pridávanie expertov zostáva rovnaký ...
        ...
        for exp in self.experts:
            ...
            # Dôsledne podávame uložený zoznam tried self._classes
